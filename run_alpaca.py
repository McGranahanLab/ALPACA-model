...
def solve(input_file_name, config):
    ...
    SS.get_solution()
    SS.optimal_solution = SS.optimal_solution
    if SS.output_all_solutions:
        SS.all_solutions = SS.get_all_simplified_solution()
    return SS
...
def main():
    ...
    parser.add_argument('--ci_table_name', default='', type=str, help='Name of file containing confidence intervals for SNP copynumbers')
    ...
    for input_file_name in input_files:
        ...
        SS.optimal_solution['run_time_seconds'] = total_run_time
        SS.optimal_solution.to_csv(output_name, index=False)
        if SS.output_all_solutions:
            SS.all_solutions.to_csv(all_solutions_output_name, index=False)
